...
const gerarAudioIAHandler = async (req, res) => {
    var _a;
    let pedidoId = null; // Variável para armazenar o ID do pedido
    try {
        ...
        const { error: debitoError } = await supabaseAdmin_1.supabaseAdmin.rpc('debitar_creditos_ia', {
            p_user_id: userId,
            p_custo_ia: custoIa,
        });
        if (debitoError) {
        ...
        const audioStream = await elevenlabs.textToSpeech.stream(ia_voice_id, {
            text: texto_roteiro,
        ...
        const relativeDir = path_1.default.join('public', 'ia_audios', userId);
        const absoluteDir = path_1.default.resolve(relativeDir);
        await promises_1.default.mkdir(absoluteDir, { recursive: true });
        const fileName = `${pedidoId}.mp3`;
        const absoluteFilePath = path_1.default.join(absoluteDir, fileName);
        await promises_1.default.writeFile(absoluteFilePath, audioBuffer);
        // 7. Gerar a URL pública local
        const publicUrl = `/${path_1.default.join('ia_audios', userId, fileName).replace(/\\/g, '/')}`;
